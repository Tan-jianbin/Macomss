...
MACOMSS.parsvd <- function(Y11, Y12, Y21) {
  ...
  M_1dot <- as.matrix(M_1dot)
  
  M_dot1 <- rbind(M11, M21)
  M_dot1 <- as.matrix(M_dot1)
...
MACOMSS <- function(Y11, Y12, Y21) {
  ...
  for (s in m:1) {
    if (!is.singular.matrix(as.matrix(B11[1:s, 1:s]))) {
      if (norm(B21[, 1:s] %*% ginv(B11[1:s, 1:s]), "2") <= 2 * sqrt(p1 / m1) |
          norm(ginv(B11[1:s, 1:s]) %*% B21[1:s, ], "2") <= 2 * sqrt(p2 / m2)) {
        hatr <- s
        break
      }
    }
...
sample_simulate_1 <- function(nr, nc) {
  ...
  beta_norandom <- c(rnorm(15), rnorm(nr - 15, 0, 0))
