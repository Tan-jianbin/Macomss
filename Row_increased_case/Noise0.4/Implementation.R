...
#####################
# Plot
result_beta <- sapply(1:length(results), function(k){
  sapply(1:(length(results[[k]]) - 1), function(l){
    mean(results[[k]][[l+1]][,1])
  })
})

result_beta <- t(result_beta)
rownames(result_beta) <- m1_values
result_beta <- data.frame(row = m1_values, result_beta)
result_beta <- data.frame(result_beta)
method_name <- c("row", "  Baseline", "  Macom", "Predictive mean matching",
                 "Random sample fromobserved values", 
                 "Classification and regression trees", "Bayesian linear regression", 
                 "K-nearest neighbors")
colnames(result_beta) <- method_name

# Convert into long data
beta_long <- result_beta %>%
...
p1 <- ggplot(beta_long) +
  geom_line(aes(x = row, y = value, color = method), size = 1) +
  labs(title = "(B)",
       x = "Number of rows",
       y = "NMSE of Regression Coefficients",
       color = "") +
...
ggsave("Figure/beta_plot.pdf", p1, width = 10, height = 6)
...
ggsave("Figure/AUC_plot.pdf", p1, width = 10, height = 6)
...
ggsave("Figure/mse_plot_log.pdf", p2, width = 10, height = 6)

# Group plot
library(patchwork)
combined <- p2 + p1 + p2 & theme(legend.position = "bottom")
