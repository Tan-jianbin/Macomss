...
method_name <- factor(c("Complete-data method", "MACOMSS", "PMM",
                 "RS", 
                 "CART", "BLR", 
                 "K-NN"), 
                 levels = c("Complete-data method", "MACOMSS", "PMM",
                 "RS", 
                 "CART", "BLR", 
                 "K-NN"))
...
p1 <- ggplot(result_auc, aes(x = Row, y = Value, color = Method)) +
  ...
  scale_color_manual(values = method_colors_auc,
                     labels = method_name, 
                     name = "Method", guide = FALSE) +
  scale_shape(guide = FALSE) +
  scale_y_sqrt(limits = c(min(result_auc$Value, na.rm = T), max(result_auc$Value, na.rm = T))) +
  labs(title = "(B)",
       x = "Number of rows",
       y = "AUC",
       color = "") 
...
p2 <- ggplot(result_mse, aes(x = Row, y = Value, color = Method)) +
  ...
  theme(panel.grid.minor = element_blank(),
        ...
        plot.title = element_text(hjust = 0.5),
        axis.text.x = element_text(angle = 0)) +
  scale_color_manual(values = method_colors_nmse,
                     labels = method_name, 
                     name = "Method", guide = FALSE) +
  scale_shape(guide = FALSE) +
...
ggsave("Combine_figure.pdf", width = 6.5, height = 4.5)
