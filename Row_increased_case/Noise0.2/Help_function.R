...
fit_and_calculate_errors <- function(data, method_name, train_label, test_label, beta, test_data, train_data,train_data_bar,XB) {
  ...
  beta_diff <- sum((beta - beta_method)^2, na.rm = TRUE) / sum(beta^2, na.rm = TRUE)
  zero_mark <- beta == 0
  ...
  mae <- ifelse(method_name != "raw" && method_name != "omit", 
                mean(abs(data - train_data), na.rm = TRUE)/mean(abs(data),na.rm = TRUE), NA)
...
parallel_simulation <- function(m1_values, m2, p1, lmax, num_cores = detectCores() - 1) {
  ...
  clusterExport(cl, c("implement_func", "sample_simulate_1",
                      ...
                      "MACOMSS.parsvd", "MACOMSS", "sample_simulate",
                      "missing_simulate", "label_binary",
                      "sigmoid_binary_y"))
